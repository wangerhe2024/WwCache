...
#include "WICachePolicy.h"

namespace WwCache {

// 前向声明
template <typename Key, typename Value> class WLruCache;
...
class WLruCache : public WICachePolicy<Key, Value> {
public:
  using LruNodeType = LruNode<Key, Value>;
  using NodePtr = std::shared_ptr<LruNodeType>;
  using NodeMap = std::unordered_map<Key, NodePtr>;

  WLruCache(int capacity) : capacity_(capacity) { initializeList(); }
...
template <typename Key, typename Value>
class WLruKCache : public WLruCache<Key, Value> {
public:
  WLruKCache(int capacity, int historyCapacity, int k)
      : WLruCache<Key, Value>(capacity) // 调用基类构造
        ,
